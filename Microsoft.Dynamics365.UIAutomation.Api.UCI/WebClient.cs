...
namespace Microsoft.Dynamics365.UIAutomation.Api.UCI
{
    public class WebClient : BrowserPage, IDisposable
    {
        ...
        internal BrowserCommandResult<bool> BC_SignOut()
        {
...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        internal BrowserCommandResult<List<GridItem>> BC_GetGridItems(int thinkTime = Constants.DefaultThinkTime)
        {
            ...
            return this.Execute(GetOptions("Get Grid Items"), driver =>
            {
                ...
                {
                    var Cells = row.FindElements(By.XPath("//form[not(@tabindex)]//td[@role='gridcell']/*[not(@title='Show more options')]"));
                    var CurrentIndex = 0;
                    var ColumnHeaders = driver.FindElements(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.Grid_Header]));
                    var GridItem = new GridItem();

                    foreach (var columnHeader in ColumnHeaders)
                        {
                        var CellText = Cells[CurrentIndex].Text;
                        try
                        {
                            GridItem[ColumnHeaders[CurrentIndex].Text] = CellText;
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"TEMPDEV WebClient.cs BC_GetGridItems Rows.Count: <{Rows.Count()}>");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() ColumnHeaders XPath: {AppElements.Xpath[AppReference.BusinessCentral.Grid_Header]}");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() CellText: <{CellText}>");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() CurrentIndex: <{CurrentIndex}>");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() Cell.Count(): <{Cells.Count()}>");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() ColumnHeaders Count: {ColumnHeaders.Count}");
                            Console.WriteLine($"TEMPDEV BC_GetGridItems() ColumnHeaders.Count(): <{ColumnHeaders.Count()}>");
                            throw;
                        }
                        CurrentIndex++;
        ...
        internal BrowserCommandResult<bool> BC_SearchRecords(string purchaseOrderNumber, int thinkTime = Constants.DefaultThinkTime)
        {
            ...
            {
                driver.WaitUntilClickable(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.InactiveSearchInRecordsButton]),
                    TimeSpan.FromSeconds(5),
                    d => { driver.ClickWhenAvailable(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.InactiveSearchInRecordsButton])); },
                    "The Search button is not available."
                );

                var input = driver.WaitUntilVisible(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.ActiveSearchInRecordsButton]));
        ...
        private BrowserCommandResult<bool> BC_UnfoldSection(string sectionName)
        {
            return this.Execute(GetOptions($"Unfold section"), driver =>
            {
                ...
                {
                    Console.WriteLine($"TEMPDEV WebClient.cs BC_UnfoldSection() SectionHeaderFolded: {AppElements.Xpath[AppReference.BusinessCentral.SectionHeaderFolded].Replace("[SECTIONNAME]", sectionName)}");
                ...
                {
                    Console.WriteLine($"TEMPDEV WebClient.cs BC_UnfoldSection() SectionHeaderFolded: {AppElements.Xpath[AppReference.BusinessCentral.SectionHeaderFolded2].Replace("[SECTIONNAME]", sectionName)}");
        ...
        internal BrowserCommandResult<string> BC_GetField(string sectionName, string fieldName)
        {
            return this.Execute(GetOptions($"Get Field Value"), driver =>
            {
                ...
                string FieldText;
                if (driver.IsVisible(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.TextFieldContainer].Replace("[SECTIONNAME]", sectionName).Replace("[FIELDNAME]", fieldName))))
                {
                    Console.WriteLine($"TEMPDEV Webclient.cs BC_GetField() FieldContainer: {AppElements.Xpath[AppReference.BusinessCentral.TextFieldContainer].Replace("[SECTIONNAME]", sectionName).Replace("[FIELDNAME]", fieldName)}");
                    FieldContainer = driver.FindElement(By.XPath(
                ...
                {
                    Console.WriteLine($"TEMPDEV Webclient.cs BC_GetField() FieldContainer2: {AppElements.Xpath[AppReference.BusinessCentral.TextFieldContainer2].Replace("[SECTIONNAME]", sectionName).Replace("[FIELDNAME]", fieldName)}");
                    FieldContainer = driver.FindElement(By.XPath(
                ...
                Console.WriteLine($"TEMPDEV Webclient.cs BC_GetField() FieldText: <{FieldText}>");
                return FieldText;
            });
        }
        ...
        internal BrowserCommandResult<string> BC_GetField(string fieldName)
        {
            return this.Execute(GetOptions($"Get Field Value"), driver =>
            {
                ...
                Console.WriteLine($"DEBUG: Webclient.cs BC_GetGenericField() FieldText: <{FieldText}>");
                return FieldText;
            });
        }
        ...
        internal BrowserCommandResult<string> Bc_GetEntityNoFromHeader()
        {
            return this.Execute(GetOptions($"Get Entity Header"), driver =>
            {
                ...
                IWebElement FieldContainer = driver.WaitUntilAvailable(By.XPath(
                                ...
                                TimeSpan.FromSeconds(1));
                Console.WriteLine($"TEMPDEV Webclient.cs Bc_GetEntityNoFromHeader() XPath: {AppElements.Xpath[AppReference.BusinessCentral.EntityHeader]}");
                Console.WriteLine($"TEMPDEV Webclient.cs Bc_GetEntityNoFromHeader() FieldContainer.Text: {FieldContainer.Text}");
                string EntityNo = FieldContainer.Text.Split(' ')[0].Trim();
                Console.WriteLine($"TEMPDEV Webclient.cs Bc_GetEntityNoFromHeader() EntityNo: {EntityNo}");
                return EntityNo;
            });
        }
        ...
        internal BrowserCommandResult<bool> BC_SetField(string sectionName, string fieldName, string fieldValue)
        {
            return Execute(GetOptions("Set Field Value"), driver =>
            {
                ...
                {
                    Console.WriteLine($"TEMPDEV Webclient.cs BC_SetField() FieldContainer: {AppElements.Xpath[AppReference.BusinessCentral.TextFieldContainer].Replace("[SECTIONNAME]", sectionName).Replace("[FIELDNAME]", fieldName)}");
                    throw new NoSuchElementException($"Field '{fieldName}' was not found on the page.");
                }
                ...
                {
                    BC_Check_DialogText("Do you want to update the lines?");
        ...
        internal BrowserCommandResult<string> BC_AutocompleteField(string sectionName, string fieldName)
        {
            return Execute(GetOptions("Trigger Field Value autocompletion"), driver =>
            {
                ...
                return AutocompleteValue(driver, fieldContainer);
            });
        }
        ...
        {
            return Execute(GetOptions("Set Line Field Value"), driver =>
            {
                BC_SwitchToEditMode();
                BC_SwitchToFullScreen();
                BC_UnfoldSection("Lines");

                // Handling for naming exceptions. Think about refactoring afterward
                switch (lineFieldName)
                {
                    case "Requested Receipt Date - Enable":
                        lineFieldName = "Requested Rcpt. Date - En_TSL";
                        break;
                    case "Direct Unit Cost Excl. GST":
                        lineFieldName = "Direct Unit Cost";
                        break;
                    case "Entry Type":
                        lineFieldName = "EntryType";
                        break;
                    case "Area Code":
                        lineFieldName = "Shortcut Dimension 1 Code";
                        break;
                    case "Project Code":
                        lineFieldName = "ShortcutDimCode3";
                        break;
                    case "Subcode Code":
                        lineFieldName = "ShortcutDimCode4";
                        break;
                    case "Cost centre Code":
                        lineFieldName = "Shortcut Dimension 2 Code";
                        break;
                    default:
                        break;
                }

                    IWebElement RowContainer = driver.ClickWhenAvailable(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.LineRowContainer]
                                                .Replace("[LINENUMBER]", lineNumber.ToString())));
                Console.WriteLine($"TEMPDEV WebClient.cs BC_SetLineField LineRowContainer: {AppElements.Xpath[AppReference.BusinessCentral.LineRowContainer].Replace("[LINENUMBER]", lineNumber.ToString())}");
                ThinkTime(1000);

                IWebElement LineFieldContainer;
                try
                {
                    LineFieldContainer = driver.FindElement(By.XPath(
                        AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                        .Replace("[LINENUMBER]", lineNumber.ToString())
                        .Replace("[FIELDNAME]", lineFieldName)+"//input"));

                    //TODO: refactoring
                    if (!LineFieldContainer.IsVisible())
                    {
                        Console.WriteLine("TEMPDEV WebClient.cs BC_SetLineField() LineFieldContainer not visible - scrolling... ");
                        IWebElement HorizontalBar = driver.FindElement(By.ClassName("freeze-pane-scrollbar"));
                        Actions Actions = new Actions(driver);
                        Actions MoveToElement = Actions.MoveToElement(HorizontalBar);
                        for (int i = 0; i < 100; i++)
                        {
                            if (LineFieldContainer.IsVisible()) Console.WriteLine($"TEMPDEV WebClient.cs BC_SetLineField() LineFieldContainer visible after {i} right key clicks.");
                            if (LineFieldContainer.IsVisible()) break;
                            MoveToElement.SendKeys(Keys.Right).Build().Perform();
                        }
                    }
                    if (!LineFieldContainer.IsVisible()) throw new NoSuchElementException($"Cannot find line field with name {lineFieldName} using following XPath: {AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit].Replace("[LINENUMBER]", lineNumber.ToString()).Replace("[FIELDNAME]", lineFieldName) + "//input"}");
                } catch (Exception)
                {
                    try
                    {
                        driver.ClickIfVisible(By.XPath(
                            AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                            .Replace("[LINENUMBER]", lineNumber.ToString())
                            .Replace("[FIELDNAME]", lineFieldName) + "//select"),
                            TimeSpan.FromSeconds(1));
                        driver.ClickIfVisible(By.XPath(
                            AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                            .Replace("[LINENUMBER]", lineNumber.ToString())
                            .Replace("[FIELDNAME]", lineFieldName) + $"//option[text()='{lineFieldValue}']"),
                            TimeSpan.FromSeconds(1));
                        Console.WriteLine($"TEMPDEV Webclient.cs BC_SetLineField() [2] XPath: {AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit].Replace("[LINENUMBER]", lineNumber.ToString()).Replace("[FIELDNAME]", lineFieldName) + "//select"}");
                        return true;
                    }
                    catch (Exception)
                    {
                        try
                        {
                            LineFieldContainer = driver.FindElement(By.XPath(
                                AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                                .Replace("[LINENUMBER]", lineNumber.ToString())
                                .Replace("[FIELDNAME]", $"{lineFieldName}_TSL") + "//input"));
                            Console.WriteLine($"TEMPDEV Webclient.cs BC_SetLineField() [3] XPath: {AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit].Replace("[LINENUMBER]", lineNumber.ToString()).Replace("[FIELDNAME]", $"{lineFieldName}_TSL") + "//input"}");
                        }
                        catch (Exception)
                        {
                            ...
                            driver.ClickIfVisible(By.XPath(
                                AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                                .Replace("[LINENUMBER]", lineNumber.ToString())
                                .Replace("[FIELDNAME]", $"{lineFieldName}_TSL") + "//select"),
                                TimeSpan.FromSeconds(1));
                            driver.ClickIfVisible(By.XPath(
                                AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit]
                                .Replace("[LINENUMBER]", lineNumber.ToString())
                                .Replace("[FIELDNAME]", $"{lineFieldName}_TSL") + $"//option[text()='{lineFieldValue}']"),
                                TimeSpan.FromSeconds(1));
                            Console.WriteLine($"TEMPDEV Webclient.cs BC_SetLineField() [4] XPath: {AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerEdit].Replace("[LINENUMBER]", lineNumber.ToString()).Replace("[FIELDNAME]", $"{lineFieldName}_TSL") + "//select"}");
                            return true;
                        }
                    }
                }
                SetInputValue(driver, LineFieldContainer, lineFieldValue);
        ...
        internal BrowserCommandResult<bool> BC_ClickMenuAndSubmenu(string menuTitle, string submenuTitle, string documentsOption)
        {
            return Execute(GetOptions($"Open Level Three Menu Item"), driver =>
            {
                ThinkTime(1000);
                ...
                BC_ClickMenuAndSubmenu(menuTitle, submenuTitle);

                driver.ClickWhenAvailable(By.XPath(
                                AppElements.Xpath[AppReference.BusinessCentral.DocumentsMenuOptionButton]
                                .Replace("[DOCUMENTSOPTION]", documentsOption)), $"The Documents Menu Option '{documentsOption}' does not exist");

                return true;
            });
        }

        internal BrowserCommandResult<bool> BC_ClickMenuAndSubmenu(string menuTitle, string submenuTitle)
        {
            try
            {
                return this.Execute(GetOptions($"Click Menu and Submenu"), driver =>
                {
                    driver.BC_WaitForPageToLoad();
                    ThinkTime(1000); //TODO improve
                    driver.FindElements(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.TopMenu].Replace("[TITLE]", menuTitle))).LastOrDefault().Click();
                    ThinkTime(1000); //TODO improve
                    driver.ClickWhenAvailable(By.XPath(AppElements.Xpath[AppReference.BusinessCentral.TopSubmenu].Replace("[TITLE]", submenuTitle)));
            ...
            {
                throw new InvalidOperationException($"Field: {AppElements.Xpath[AppReference.BusinessCentral.TopMenu].Replace("[TITLE]", menuTitle)} or {AppElements.Xpath[AppReference.BusinessCentral.TopSubmenu].Replace("[TITLE]", submenuTitle)} Does not exist", e);
            }
        }
        ...
        internal BrowserCommandResult<bool> BC_ClickLineMenuAndSubmenu(string lineMenuTitle, string lineMenuSubTitle, string function)
        {
            try
            {
                return this.Execute(GetOptions($"Click Line Menu and Submenu"), driver =>
                {
                    // WIP
                    driver.BC_WaitForPageToLoad();
                    ThinkTime(1000); //TODO improve
        ...
        {
            return this.Execute(GetOptions($"Click OK button"), driver =>
            {
                driver.BC_WaitForPageToLoad();
                ThinkTime(1000); //TODO: performance improvement
        ...
        internal BrowserCommandResult<string> BC_Dialog_GetText()
        {
            return this.Execute(GetOptions($"Get Dialog text"), driver =>
            {
                ...
                {
                    Console.WriteLine($"[{DateTime.Now.ToString("T")}] WARNING: WebClient.cs BC_Dialog_GetText TextContainer NOT FOUND");
                    return "";
                }
            });
        }
        ...
        private BrowserCommandResult<string> BC_ExceptionDialog_GetText()
        {
        ...
        internal BrowserCommandResult<bool> BC_Check_ExceptionDialogText(string expectedExceptionDialogText)
        {
            var ActualExceptionDialogText = BC_ExceptionDialog_GetText().Value;
            if (!ActualExceptionDialogText.Contains(expectedExceptionDialogText))
            {
                throw new InvalidOperationException($"Unexpected exception dialog message: '{ActualExceptionDialogText}'");
            }
            return true;
        }
        ...
        internal BrowserCommandResult<string> BC_GetPageErrorMessage()
        {
            return this.Execute(GetOptions($"Get Page Error text"), driver =>
            {
                ...
                catch (Exception)
                {
                    try
                    {
                        Console.WriteLine($"TEMPDEV WebClient.cs BC_GetPageErrorMessage() TextContainer Not found -  Switching to Default Content...");
                        driver.SwitchTo().DefaultContent();
                        IWebElement TextContainer = driver.WaitUntilAvailable(By.XPath(
                                AppElements.Xpath[AppReference.BusinessCentral.PageError_TextContainer]),
                                TimeSpan.FromSeconds(1));
                    ...
                    {
                        Console.WriteLine($"TEMPDEV WebClient.cs BC_GetPageErrorMessage() TextContainer NOT FOUND - returning empty string.");
                        return "";
                    }
                }
            });
        ...
        internal BrowserCommandResult<bool> BC_WorkingOnItDialogHandler()
        {
            ...
            {          
                ...
                while (TotalTimeoutInMilliseconds < 10000)
                {
                    if (BC_ExceptionDialog_GetText().Value.Equals("Working on it..."))
                    {
                        Console.WriteLine($"[{DateTime.Now.ToString("T")}] INFO: Waiting for the 'Working on it' dialog to be closed...");
                        WorkingOnItWasDisplayed = true;
                    } else if (BC_ExceptionDialog_GetText().Value.Equals("") && WorkingOnItWasDisplayed)
                    {
                        Console.WriteLine($"[{DateTime.Now.ToString("T")}] INFO: 'Working on it' dialog disappeared.");
                        break;
                    } else if (!WorkingOnItWasDisplayed && TotalTimeoutInMilliseconds==1000)
                    {
                        Console.WriteLine($"[{DateTime.Now.ToString("T")}] INFO: Waited 1 second for 'Working on it' dialog but it wasn't displayed.");
                        break;
                    }
        ...
        {
            return this.Execute(GetOptions($"Click Radio option"), driver =>
            {
                driver.BC_WaitForPageToLoad();
        ...
        internal BrowserCommandResult<bool> BC_EmailVendor(SecureString email)
        {
            try
            {
                return this.Execute(GetOptions($"Email Vendor"), driver =>
                {
                    ...
                    SetInputValue(driver, input, email.ToUnsecureString());
        ...
        internal BrowserCommandResult<string> BC_GetLineField(int lineNumber, string lineFieldName)
        {
            return this.Execute(GetOptions($"Get Line Field"), driver =>
            {
                ...
                {
                    RowHeadersNames.Add(rowHeader.GetAttribute("abbr"));
                }

                if (RowHeadersNames.IndexOf(lineFieldName)==-1)
                {
                    Console.WriteLine($"TEMPDEV RowHeadersNames: {String.Join("\n", RowHeadersNames)}");
                    Console.WriteLine($"TEMPDEV WebClient.cs BC_GetLineField() LineFieldContainer: {AppElements.Xpath[AppReference.BusinessCentral.LineHeaders]}");
                    throw new NoSuchElementException($"Line field with name {lineFieldName} does not match any of the found Row Header Names.");
                }

                var ColumnIndex = RowHeadersNames.IndexOf(lineFieldName)+2;

                IWebElement LineFieldContainer;
                try
                {
                    LineFieldContainer = driver.FindElement(By.XPath(
                    AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerRead]
                    .Replace("[LINENUMBER]", lineNumber.ToString())
                    .Replace("[COLUMNINDEX]", ColumnIndex.ToString())));
                }
                catch (Exception)
                {
                    Console.WriteLine($"TEMPDEV WebClient.cs BC_GetLineField() LineFieldContainer: {AppElements.Xpath[AppReference.BusinessCentral.LineTextFieldContainerRead].Replace("[LINENUMBER]", lineNumber.ToString()).Replace("[COLUMNINDEX]", ColumnIndex.ToString())}");
                    throw new NoSuchElementException($"Line field with name {lineFieldName} does not exist.");
                }
                Console.WriteLine($"TEMPDEV WebClient.cs BC_GetLineField() LineFieldContainer.GetAttribute('title''): <{LineFieldContainer.GetAttribute("title")}>");
                var FieldValue = LineFieldContainer.GetAttribute("title").Replace("Open record", "").Replace("Show more about", "").Replace("\"", "").Trim();
                Console.WriteLine($"TEMPDEV WebClient.cs BC_GetLineField() FieldValue: <{FieldValue}>");
                return FieldValue;
            });
        }
        ...
        private string AutocompleteValue(IWebDriver driver, IWebElement input, TimeSpan? thinktime = null)
        {
            // Repeat set value if expected value is not set
            ...
            Console.WriteLine($"INFO: AutocompleteValue: {input.GetAttribute("value")}");
            driver.WaitForTransaction();
